   ...
   public:
            ...
            switch(x->tag) {
                case T_select:
                {
                    ...
                    if (auto a = std::dynamic_pointer_cast<AggregatePlan>(x->subplan_)) {
                        std::vector<TabCol> sel_cols;
                        for (auto &agg : a->agg_funcs_) {
                            sel_cols.push_back({"", agg->alias});
                        }
                    ...
                    throw InternalError("Unexpected select subplan");
                }
    ...
    std::unique_ptr<AbstractExecutor> convert_plan_executor(std::shared_ptr<Plan> plan, Context *context)
    {
        if(auto x = std::dynamic_pointer_cast<ProjectionPlan>(plan)){
            ...
            return std::make_unique<AggregateExecutor>(convert_plan_executor(x->subplan_, context), x->agg_funcs_);
