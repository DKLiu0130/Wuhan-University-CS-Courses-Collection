...
{
    public:
        AggregatePlan(PlanTag tag, std::shared_ptr<Plan> subplan, std::vector<std::shared_ptr<ast::AggFunc>> agg_funcs)
        {
            Plan::tag = tag;
            subplan_ = std::move(subplan);
...
{
    public:
        SortPlan(PlanTag tag, std::shared_ptr<Plan> subplan, std::vector<TabCol> sel_cols, std::vector<bool> is_descs, int limit)
        {
            Plan::tag = tag;
            subplan_ = std::move(subplan);
            sel_cols_ = std::move(sel_cols);
