...
bool Planner::get_index_cols(std::string tab_name, std::vector<Condition> curr_conds, std::vector<std::string>& index_col_names) {
    ...
    for (auto &index : tab.indexes) {
        ...
        for (auto &col : index.cols) {
            ...
            bool has_eq = false;
            for (auto &cond : curr_conds) {
                if (cond.is_rhs_val && cond.lhs_col.tab_name == tab_name && cond.lhs_col.col_name == col.name && cond.op == OP_EQ) {
                    has_eq = true;
                    break;
                }
            }
...
std::shared_ptr<Plan> Planner::generate_sort_plan(std::shared_ptr<Query> query, std::shared_ptr<Plan> plan)
{
    ...
    for (size_t i = 0; i < x->order->cols.size(); ++i) {
        TabCol target = {.tab_name = x->order->cols[i]->tab_name, .col_name = x->order->cols[i]->col_name};
        bool found = false;
        ...
        is_descs.push_back(x->order->orderby_dirs[i] == ast::OrderBy_DESC);
    }
    return std::make_shared<SortPlan>(T_Sort, std::move(plan), sel_cols, is_descs, x->order->limit);
}
...
std::shared_ptr<Plan> Planner::generate_select_plan(std::shared_ptr<Query> query, Context *context) {
    ...
    if (!query->agg_funcs.empty()) {
        plannerRoot = std::make_shared<AggregatePlan>(T_Aggregate, std::move(plannerRoot), query->agg_funcs);
    } else {
