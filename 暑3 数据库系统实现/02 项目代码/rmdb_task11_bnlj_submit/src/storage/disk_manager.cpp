...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    if (!fd2unixfd_.count(fd)) {
        throw FileNotOpenError(fd);
    }
    int unix_fd = fd2unixfd_[fd];
    off_t file_offset = static_cast<off_t>(page_no) * PAGE_SIZE;
    int written = 0;
    while (written < num_bytes) {
        ssize_t bytes_write = pwrite(unix_fd, offset + written, num_bytes - written, file_offset + written);
        if (bytes_write < 0) {
            throw UnixError();
        }
        if (bytes_write == 0) {
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    while (read_bytes < num_bytes) {
        ssize_t bytes_read = pread(unix_fd, offset + read_bytes, num_bytes - read_bytes, file_offset + read_bytes);
        if (bytes_read < 0) {
            throw UnixError();
        }
        if (bytes_read == 0) {
...
void DiskManager::create_file(const std::string &path) {
    ...
    if (path2id_.count(path)) {
        fd2pageno_[path2id_[path]] = 0;
    }
...
int DiskManager::open_file(const std::string &path) {
    ...
    int unix_fd = open(path.c_str(), O_RDWR);
    if (unix_fd == -1) {
        throw UnixError();
    }
    int fd;
...
int DiskManager::read_log(char *log_data, int size, int offset) {
    ...
    if(size == 0) return 0;
    int unix_fd = fd2unixfd_[log_fd_];
    lseek(unix_fd, offset, SEEK_SET);
    ssize_t bytes_read = read(unix_fd, log_data, size);
...
void DiskManager::write_log(char *log_data, int size) {
    ...
    int unix_fd = fd2unixfd_[log_fd_];
    lseek(unix_fd, 0, SEEK_END);
    ssize_t bytes_write = write(unix_fd, log_data, size);
    if (bytes_write != size) {
        throw UnixError();
    }
}
