// Experiment 3 starter: implement serial and OpenMP image convolution.
//
// What is provided:
//   1. Dataset reading: image, kernel, and expected output are loaded from conv_data.bin.
//   2. Correctness checks: your serial and OpenMP outputs are compared with expected output.
//   3. Performance test: both versions are timed and a speedup is printed.
//
...
//
// Build on Windows:
//   MinGW/MSYS2: g++ -O2 -std=c++17 -fopenmp conv_openmp_starter.cpp -o conv_openmp.exe
//   MSVC:        cl /O2 /EHsc /openmp conv_openmp_starter.cpp /Fe:conv_openmp.exe
//
// Run:
//   .\conv_openmp.exe 4
//   .\conv_openmp.exe 8 conv_data.bin
...
// TODO 1:
// Implement the single-thread convolution.
//
// Requirements:
//   - Write every element of out.
//   - Use image_index, kernel_index, and out_index to avoid index mistakes.
//   - Do not use OpenMP in this function.
            ...
            for (int c = 0; c < shape.c; ++c) {
                for (int kh = 0; kh < shape.kh; ++kh) {
                    for (int kw = 0; kw < shape.kw; ++kw) {
                        sum += image[image_index(shape, c, oh + kh, ow + kw)] * kernel[kernel_index(shape, c, kh, kw)];
                    }
                }
            }
...
// TODO 2:
// Implement the OpenMP convolution.
//
// Requirements:
//   - The result must match conv_serial and expected output.
//   - Parallelize the output space, such as the oh loop or oh/ow loops.
//   - Keep the accumulation variable local to each output element.
