...
#elif defined(RT_USING_BUDDY_AS_HEAP)
extern rt_buddy_t rt_buddy_init(const char *name, void *start_addr, rt_size_t size);
extern void *rt_buddy_alloc(rt_buddy_t buddy, rt_size_t size);
extern void rt_buddy_free(rt_buddy_t buddy, void *ptr);
extern void *rt_buddy_realloc(rt_buddy_t buddy, void *ptr, rt_size_t newsize);
extern void rt_buddy_info(rt_buddy_t buddy, rt_size_t *total, rt_size_t *used, rt_size_t *max_used);

static rt_buddy_t system_heap;
rt_inline void _buddy_info(rt_size_t *total,
    rt_size_t *used, rt_size_t *max_used)
{
    rt_buddy_info(system_heap, total, used, max_used);
}
#define _MEM_INIT(_name, _start, _size) \
    system_heap = rt_buddy_init(_name, _start, _size)
#define _MEM_MALLOC(_size)  \
    rt_buddy_alloc(system_heap, _size)
#define _MEM_REALLOC(_ptr, _newsize)    \
    rt_buddy_realloc(system_heap, _ptr, _newsize)
#define _MEM_FREE(_ptr) \
    rt_buddy_free(system_heap, _ptr)
#define _MEM_INFO(_total, _used, _max)  \
    _buddy_info(_total, _used, _max)
#else
#define _MEM_INIT(...)
